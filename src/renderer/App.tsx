...
function App() {
  ...
  useEffect(() => {
    void window.clipnest.getHistory().then((nextItems) => setItems(nextItems));
    return window.clipnest.onHistoryUpdated((nextItems) => setItems(nextItems));
  }, []);
  ...
  }, [copyItem, filteredItems, selectedItem]);
      ...
      <header className="paste-topbar drag-region">
        ...
        <div className="paste-toolbar no-drag">
          ...
          <div className="top-filter-stack">
            ...
            <button
              className={`top-chip utility-chip ${activeFilter === "link" ? "active" : ""}`}
              onClick={() => setActiveFilter("link")}
            >
              <Circle className="chip-dot link-dot" size={9} fill="currentColor" strokeWidth={0} />
              实用链接
            </button>
          </div>
          <button
        ...
        <button className="more-button no-drag" aria-label="更多选项"><MoreHorizontal size={17} /></button>
      </header>
      ...
      <main className="paste-content">
        <div className="view-toolbar no-drag">
          ...
          {items.length === 0 ? (
            ...
            <section className="paste-grid" role="listbox" aria-label="剪切板历史">
              {filteredItems.map((item, index) => (
                <HistoryCard
                  ...
                  onDelete={() => void window.clipnest.deleteItem(item.id)}
                  onPin={() => void window.clipnest.togglePinItem(item.id)}
                />
              ))}
...
function HistoryCard({
  ...
  return (
    <article
      className={`history-card type-${item.type} ${selected ? "selected" : ""}`}
      role="option"
    ...
    >
      ...
      <div className="card-content">
        {item.type === "image" ? (
          <div className="image-card-preview">
            <img src={item.content} alt={item.preview} />
          </div>
        ) : item.type === "link" ? (
      ...
      <div className="card-footer">
        ...
        <span>{item.pinned ? "已固定" : `#${String(index + 1).padStart(2, "0")}`}</span>
      </div>
      <div className="card-actions no-drag">
        <button onClick={(event) => { event.stopPropagation(); onPin(); }} className={item.pinned ? "is-pinned" : ""} aria-label={item.pinned ? "取消固定" : "固定"}>
